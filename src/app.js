...
App = {
    ...
    getDoctorInsurances: async (total) => {
      console.log(App.account);
      App.setLoading(true)
      var content = "<table><tr><th>Id</th><th>Doctor Name</th><th>Reason</th><th>Status(is approved)</th><th>Amount</th><th>Action</th></tr>"
      for(i=0; i<total; i++){
        let y,z;
        const res = await App.Auth.getUserInsuranceById(i,{from:App.account}).then(function(x){
        y=x;
        console.log(x,y[0]);
      });
      if(y[3]=='true'){
          content += '<tr><td>' + y[0] + '</td><td>' + y[1] + '</td><td>' + y[2] + '</td><td>' + y[3] +  '</td><td>' + y[4] + '</td></tr>';
      }else{
        content += '<tr><td>' + y[0] + '</td><td>' + y[1] + '</td><td>' + y[2] + '</td><td>' + y[3] +  '</td><td>' + y[4] + '</td><td><button val=' +y[0]  + ' onclick="App.approveInsurance('+ y[0]+')">Approve</button><button val='+y[0]  + ' onclick="App.disapproveInsurance('+ y[0]+')">Disapprove</button></td></tr>';
    ...
    getUserInsurances: async (total) => {
      ...
      var content = "<table><tr><th>Id</th><th>Doctor Name</th><th>Reason</th><th>Status</th><th>Amount</th></tr>"
      for(i=0; i<total; i++){
        let y,z;
        const res = await App.Auth.getUserInsuranceById(i,{from:App.account}).then(function(x){
        y=x;
      ...
      if(loggedin){
        ...
        var option = '';
         for (var i=0;i < doctors2.length;i++){
          ...
          option += '<option value="'+ Object.keys(doctorsObject[i])[0]+'">' + doctors2[i] + '</option>';
        }
    ...
    register: async () => {
      ...
      await App.Auth.register(address,draddress,username,psw,isDr,{from:App.account});
    ...
    createInsurance: async () => {
      ...
      await App.Auth.createInsurance(App.account,reason,amount,{from:App.account});
    ...
    approveInsurance: async (id) => {
      let y;
      const res = await App.Auth.getUserInsuranceById(id,{from:App.account}).then(function(x){
        y=x;
...
const signPromise = await web3.eth.accounts.signTransaction(tx, 'e2a96e5c991f3e1f36f3c2efc1b6e017b9fc5653d7cbedbae49953d70f50b3ca').then((signedTx) => {
...
console.log(web3.utils.isAddress('0xdD054eA909E8e8cb9ddCdCDd5978E96b8D96326b') ); 
